    ...
    render() {
        ...
        if (defined(this.props.sourceNames)) {
            ...
            if (this.props.canDownload) {
                ...
                downloadButton = <Dropdown selectOption={this.downloadDropdown} options={nameAndHrefObjects} theme={downloadDropdownTheme}>下载&nbsp;▾</Dropdown>;
            }
        ...
        return (
            <div className={Styles.chartExpand}>
                <button type='button' className={Styles.btnChartExpand} onClick={this.expandButton}>更多</button>{downloadButton}
            </div>
        );
    }
...
function expand(props, sourceIndex) {
    ...
    const group = terria.catalog.upsertCatalogGroup(CatalogGroup, '图表数据', '一个图表数据的组.');
    group.isOpen = true;
