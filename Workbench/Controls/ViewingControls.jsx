...
const ViewingControls = createReactClass({
    ...
    splitItem() {
        ...
        serializedItem.name = serializedItem.name + ' (复制)';
        ...
        return (
            <ul className={Styles.control}>
                <If condition={item.canZoomTo}>
                    <li className={classNames(Styles.zoom, classList)}><button type='button' onClick={this.zoomTo} title="缩放至内容" className={Styles.btn}>缩放至内容</button></li>
                </If>
                <If condition={item.tableStructure && item.tableStructure.sourceFeature}>
                    <li className={classNames(Styles.zoom, classList)}><button type='button' onClick={this.openFeature} title="缩放至数据" className={Styles.btn}>缩放至数据</button></li>
                </If>
                <If condition={item.showsInfo}>
                    <li className={classNames(Styles.info, classList)}><button type='button' onClick={this.previewItem} className={Styles.btn} title='信息'>数据信息</button></li>
                </If>
                <If condition={canSplit}>
                    <li className={classNames(Styles.split, classList)}><button type='button' onClick={this.splitItem} title="复制及显示分帘工具" className={Styles.btn}>分帘</button></li>
                </If>
                <li className={classNames(Styles.remove, classList)}>
                    <button type='button' onClick={this.removeFromMap} title="移除数据" className={Styles.btn}>移除<Icon glyph={Icon.GLYPHS.remove}/></button>
                </li>
            </ul>
        );
    },
