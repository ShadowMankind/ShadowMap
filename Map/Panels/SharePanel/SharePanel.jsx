...
const SharePanel = createReactClass({
    ...
    beforeBrowserPrint() {
        ...
        this._message.innerText = '为了更好的打印效果, 请使用' + this.props.terria.appName + '的打印按钮代替你的网页浏览器的打印功能.';
        ...
        if (this.shouldShorten()) {
            ...
            buildShortShareLink(this.props.terria)
                ...
                .otherwise(() => {
                    ...
                    this.setState({
                        errorMessage: '在缩短URL中发生一个错误.  请检查你的网络连接并再次尝试.'
                    });
                });
        } else {
            this.setUnshortenedUrl();
        ...
        return (
            <div>
                ...
                <div className={DropdownStyles.section}>
                    <div>打印地图</div>
                    <div className={Styles.explanation}>打开此地图的可打印版本.</div>
                    <div>
                        <button className={Styles.printButton} onClick={this.print} disabled={this.state.creatingPrintView}>打印</button>
                        <button className={Styles.printButton} onClick={this.showPrintView} disabled={this.state.creatingPrintView}>显示打印视图</button>
                        <div className={Styles.printViewLoader}>
                            {this.state.creatingPrintView && <Loader message="正在创建打印视图..." />}
                        </div>
                    </div>
                </div>
                <div className={classNames(DropdownStyles.section, Styles.shortenUrl)}>
                    ...
                    <If condition={this.advancedIsOpen()}>
                        <div className={DropdownStyles.section}>
                            <p className={Styles.paragraph}>要嵌入，请复制此段代码将地图嵌入至HTML页面中:</p>
